...
- License: GNU General Public License version 2 or later for the FCEUX core;
  incorporated components retain the compatible licenses documented in
...
- Vendored copy: `Sources/CFCEUX/fceux/` — a subset of upstream `src/`:
  ...
  - Excluded: `drivers/` (Qt/SDL/win frontends), `lua/` + `lua-engine.cpp`, `attic/`,
    `utils/ConvertUTF.c` (unused without libarchive; its licensed header remains
    for a Windows-only conditional path in `xstring.cpp`), `fir/` (build-time tool + data)
  - `palettes/conv.cpp` copied but excluded from the build (standalone tool, defines
    duplicate symbols)

### Local modifications

- 2026-08-11: Added `Sources/CFCEUX/fceux/drivers/sdl/sdl.h` — a stub. `fceu.cpp` unconditionally
...
- 2026-08-11: `Sources/CFCEUX/fceux/x6502.cpp` removes debugger/test counters from the CPU
